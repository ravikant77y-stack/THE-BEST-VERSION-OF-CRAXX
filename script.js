...
// --------------------------------------------------------
const ROUTER_VIEWS = [
  'dashboard', 'store-manager', 'targets', 'orders', 'products', 'product-manager',
...
let craxxSupabaseClient = null;
let craxxOtpPendingEmail = "";

function getAuthSettings() {
  return readJson("craxxAuthConfig", { url: "", anonKey: "" });
}

function getSupabaseClient() {
  if (craxxSupabaseClient) return craxxSupabaseClient;
  let config = getAuthSettings();
  if (!config.url || !config.anonKey || !window.supabase?.createClient) return null;
  craxxSupabaseClient = window.supabase.createClient(config.url, config.anonKey, {
    auth: { persistSession: true, autoRefreshToken: true }
  });
  return craxxSupabaseClient;
}
...
function activateCraxxUser(userId, email, mode) {
  localStorage.setItem("craxxActiveUserId", userId || "local-admin");
  localStorage.setItem("craxxActiveUserEmail", email || "Local Administrator");
  localStorage.setItem("craxxAuthMode", mode || "local");
...
async function checkLoginSession() {
  ...
  if (client) {
    try {
      ...
      if (user) {
        activateCraxxUser(user.id, user.email, "otp");
        overlay.hidden = true;
        handleHashRoute();
        return;
      }
    } catch (error) {
      setLoginMessage("Could not verify the email session. Local access is still available.", "error");
    }
  }

  let localSession = sessionStorage.getItem("craxxLoggedIn") === "yes";
  let rememberedLocal = localStorage.getItem("craxxRememberMe") === "yes"
    && localStorage.getItem("craxxAuthMode") === "local";
  if (localSession || rememberedLocal) {
    if (!localStorage.getItem("craxxActiveUserId")) {
      activateCraxxUser("local-admin", "Local Administrator", "local");
    }
    overlay.hidden = true;
    handleHashRoute();
  } else {
    overlay.hidden = false;
    let configured = Boolean(getAuthSettings().url && getAuthSettings().anonKey);
    if (!configured) {
      setLoginMessage("Email OTP needs Supabase details in Settings. Use Local Administrator to configure it.", "info");
    }
  }
}
...
async function requestEmailOtp() {
  ...
  if (!email) {
    setLoginMessage("Enter your email address first.", "error");
    return;
  }
  let client = getSupabaseClient();
  if (!client) {
    setLoginMessage("Email OTP is not configured yet. Sign in locally and add Supabase details in Settings.", "error");
    return;
  }

  let button = document.getElementById('loginSubmitBtn');
  ...
  try {
    ...
    craxxOtpPendingEmail = email;
    document.getElementById('loginOtpLabel').hidden = false;
    document.getElementById('loginOtp').required = true;
    document.getElementById('loginResendBtn').hidden = false;
    document.getElementById('loginEmail').readOnly = true;
    button.textContent = "Verify OTP";
    setLoginMessage(`OTP sent to ${email}. Check your inbox and spam folder.`, "success");
  } catch (error) {
    button.textContent = "Send Email OTP";
    setLoginMessage(error.message || "Unable to send OTP.", "error");
  } finally {
...
async function verifyEmailOtp() {
  let client = getSupabaseClient();
  let email = craxxOtpPendingEmail || document.getElementById('loginEmail').value.trim();
  let token = document.getElementById('loginOtp').value.trim();
  if (!client || !email || !token) {
    setLoginMessage("Enter the OTP sent to your email.", "error");
    return;
  }
  ...
  try {
    ...
    if (!user) throw new Error("OTP verified but no user session was returned.");
    activateCraxxUser(user.id, user.email, "otp");
    document.getElementById('loginOverlay').hidden = true;
    recordActivity("User Signed In", user.email);
    handleHashRoute();
  } catch (error) {
    setLoginMessage(error.message || "Invalid or expired OTP.", "error");
    button.textContent = "Verify OTP";
  } finally {
    button.disabled = false;
  }
...
function handleCraxxLogin(event) {
  ...
  if (document.getElementById('loginOtpLabel').hidden) {
    requestEmailOtp();
  } else {
    verifyEmailOtp();
  }
...
function signInLocalAdmin() {
  activateCraxxUser("local-admin", "Local Administrator", "local");
  document.getElementById('loginOverlay').hidden = true;
  recordActivity("Local Administrator Signed In", "Offline workspace");
  handleHashRoute();
}

async function logOutUser() {
  let email = localStorage.getItem("craxxActiveUserEmail") || "Current user";
  if (localStorage.getItem("craxxAuthMode") === "otp") {
    try {
  ...
  recordActivity("User Logged Out", email);
  ...
  localStorage.removeItem("craxxActiveUserId");
  localStorage.removeItem("craxxActiveUserEmail");
  localStorage.removeItem("craxxAuthMode");
...
function renderDashboardView() {
  ...
  let periodLabel = dashboardPeriod === 'today' ? "Today's" : (dashboardPeriod === 'weekly' ? "This Week's" : "This Month's");
  document.getElementById('kpiRevenueTitle').textContent = `${periodLabel} Revenue`;
  document.getElementById('kpiProfitTitle').textContent = `${periodLabel} Profit`;
  document.getElementById('kpiOrdersTitle').textContent = `${periodLabel} Orders`;
  animateNumber(document.getElementById('kpiProfit'), source.profit, "₹");
  ...
  let topRows = Object.values(source.products || {})
    .map(product => {
      let catalogProduct = appAnalytics.products[product.name] || {};
      let inventoryProduct = appAnalytics.productRows.find(item => item.name === product.name) || {};
      return { ...catalogProduct, ...inventoryProduct, ...product };
    })
  ...
  document.getElementById('dashboardProductRanking').innerHTML = topRows.map((p, idx) => {
    ...
    `;
  }).join('') || `<tr><td colspan="5" style="text-align:center; color:var(--text-muted);">No products sold in this period.</td></tr>`;
...
function renderStoreManagerView() {
  ...
  let setMetric = (id, value, money = false) => {
    let element = document.getElementById(id);
    if (element) element.textContent = money ? formatMoney(value) : Number(value || 0).toLocaleString("en-IN");
  };

  setMetric('managerTodayRevenue', appAnalytics.today.revenue, true);
  setMetric('managerTodayOrders', appAnalytics.today.orders);
  setMetric('managerTodayProfit', appAnalytics.today.profit, true);
  setMetric('managerTodayUnits', appAnalytics.today.units);
  setMetric('managerWeekRevenue', appAnalytics.weekly.revenue, true);
  setMetric('managerWeekOrders', appAnalytics.weekly.orders);
  setMetric('managerWeekProfit', appAnalytics.weekly.profit, true);
  setMetric('managerMonthRevenue', appAnalytics.monthly.revenue, true);
  setMetric('managerMonthOrders', appAnalytics.monthly.orders);
  setMetric('managerMonthProfit', appAnalytics.monthly.profit, true);

  drawLineChart(document.getElementById('managerRevenueChart'), appAnalytics.monthly.series, 'revenue', '#6366f1');
  drawLineChart(document.getElementById('managerProfitChart'), appAnalytics.monthly.series, 'profit', '#10b981');

  let inventory = appAnalytics.inventory;
  let total = Math.max(inventory.total, 1);
  let healthyPercent = (inventory.healthy / total) * 100;
  let lowPercent = (inventory.low / total) * 100;
  document.getElementById('managerInventoryPie').style.background =
    `conic-gradient(var(--color-success) 0 ${healthyPercent}%, var(--color-warning) ${healthyPercent}% ${healthyPercent + lowPercent}%, var(--color-danger) ${healthyPercent + lowPercent}% 100%)`;
  document.getElementById('managerInventoryLegend').innerHTML = `
    <span><i class="healthy"></i>Healthy (${inventory.healthy})</span>
    <span><i class="low"></i>Low (${inventory.low})</span>
    <span><i class="out"></i>Out (${inventory.out})</span>
  `;

  let products = [...appAnalytics.productRows];
  let listMarkup = (rows, valueFactory) => rows.map((product, index) => `
    <div class="manager-list-item">
      <span><b>${index + 1}. ${product.name}</b><small>${valueFactory(product)}</small></span>
      <span class="status-pill ${product.currentStock <= 0 ? 'out-of-stock' : (product.currentStock <= product.reorderLevel ? 'low-stock' : 'healthy')}">${product.currentStock}</span>
    </div>
  `).join('') || `<div class="manager-empty">No matching products.</div>`;

  let lowest = products.filter(product => product.currentStock > 0)
    .sort((a, b) => a.currentStock - b.currentStock)
    .slice(0, 5);
  let out = products.filter(product => product.currentStock <= 0).slice(0, 8);
  let fast = [...products].sort((a, b) => b.units - a.units).slice(0, 5);
  let slow = [...products].sort((a, b) => a.units - b.units || a.name.localeCompare(b.name)).slice(0, 5);

  document.getElementById('managerLowestStock').innerHTML = listMarkup(lowest, product => `Reorder at ${product.reorderLevel}`);
  document.getElementById('managerOutOfStock').innerHTML = listMarkup(out, () => 'No stock available');
  document.getElementById('managerFastProducts').innerHTML = listMarkup(fast, product => `${product.units} units sold this month`);
  document.getElementById('managerSlowProducts').innerHTML = listMarkup(slow, product => `${product.units} units sold this month`);

  document.getElementById('managerTopProducts').innerHTML = fast.map(product => `
    <tr>
  ...
  `).join('') || `<tr><td colspan="4" style="text-align:center;">No sales this month.</td></tr>`;

  document.getElementById('managerActivityFeed').innerHTML = appAnalytics.activity.slice(0, 8).map(item => `
    <div class="activity-item">
      <strong>${item.action}</strong>
      <span>${item.detail || ""}</span>
      <time>${new Date(item.time).toLocaleString("en-IN")}</time>
    </div>
  `).join('') || `<div class="manager-empty">No recent activity.</div>`;
}

let activeTargetPeriod = 'today';

function getSalesTargets() {
  return readJson("salesTargets", {
    revenue: { today: 0, weekly: 0, monthly: 0 },
    products: []
  });
}

function getTargetDaysRemaining(period) {
  let now = new Date();
...
function setTargetPeriod(period, btn) {
  activeTargetPeriod = period;
  if (btn) {
    document.querySelectorAll('#targetPeriodTabs button').forEach(button => button.classList.remove('active'));
    btn.classList.add('active');
  }
  let formPeriod = document.getElementById('targetFormPeriod');
  let productPeriod = document.getElementById('targetProductPeriod');
  if (formPeriod) formPeriod.value = period;
  if (productPeriod) productPeriod.value = period;
  renderTargetsView();
}

function populateTargetProducts() {
  let select = document.getElementById('targetProductSelect');
  if (!select) return;
  let current = select.value;
  select.innerHTML = Object.values(getProductCatalog())
    .sort((a, b) => a.name.localeCompare(b.name))
    .map(product => `<option value="${product.sku}">${product.name} - ${formatMoney(product.price)}</option>`)
    .join('');
...
function renderTargetsView() {
  ...
  let source = appAnalytics[activeTargetPeriod] || appAnalytics.today;
  let goal = Number(targets.revenue?.[activeTargetPeriod] || 0);
  let achieved = Number(source.revenue || 0);
  let remaining = Math.max(0, goal - achieved);
  let percent = goal > 0 ? Math.min(100, (achieved / goal) * 100) : 0;
  let daysRemaining = getTargetDaysRemaining(activeTargetPeriod);
  let runRate = remaining / Math.max(daysRemaining, 1);
  let periodText = activeTargetPeriod === 'today' ? 'Today' : (activeTargetPeriod === 'weekly' ? 'This Week' : 'This Month');

  document.getElementById('targetRevenueGoal').textContent = formatMoney(goal);
  document.getElementById('targetRevenueAchieved').textContent = formatMoney(achieved);
  document.getElementById('targetRevenueRemaining').textContent = formatMoney(remaining);
  document.getElementById('targetDailyRunRate').textContent = formatMoney(runRate);
  document.getElementById('targetPeriodLabel').textContent = periodText;
  document.getElementById('targetAchievementPercent').textContent = `${Math.round(percent)}% achieved`;
  document.getElementById('targetDaysRemaining').textContent = `${daysRemaining} day${daysRemaining === 1 ? '' : 's'} remaining`;
  document.getElementById('targetProgressPercent').textContent = `${Math.round(percent)}%`;
  document.getElementById('targetProgressGoal').textContent = formatMoney(goal);
  document.getElementById('targetProgressBar').style.width = `${percent}%`;
  document.getElementById('targetProgressStatus').textContent = goal <= 0 ? 'Set a target to begin' : (remaining <= 0 ? 'Target achieved' : `${formatMoney(remaining)} remaining`);

  let catalog = getProductCatalog();
  let averagePrice = Object.values(catalog).reduce((sum, product) => sum + Number(product.price || 0), 0) / Math.max(Object.keys(catalog).length, 1);
  let estimatedUnits = averagePrice > 0 ? Math.ceil(remaining / averagePrice) : 0;
  document.getElementById('targetGuidance').innerHTML = goal <= 0
    ? `<p>Set a revenue target to receive daily sales guidance.</p>`
    : remaining <= 0
      ? `<p><strong>Target achieved.</strong> You are ${formatMoney(achieved - goal)} above the goal.</p>`
      : `<p>Sell approximately <strong>${estimatedUnits} average-priced units</strong>, or generate <strong>${formatMoney(runRate)} per remaining day</strong>, to reach this target.</p>`;

  let periodTargets = (targets.products || []).filter(target => target.period === activeTargetPeriod);
  let table = document.getElementById('productTargetTableBody');
  table.innerHTML = periodTargets.map(target => {
    let product = Object.values(catalog).find(item => item.sku === target.sku);
    if (!product) return '';
    let sold = Number(source.products?.[product.name]?.units || 0);
    let targetQty = Number(target.quantity || 0);
    let qtyRemaining = Math.max(0, targetQty - sold);
    let productPercent = targetQty > 0 ? Math.min(100, (sold / targetQty) * 100) : 0;
    return `
      <tr>
        <td><strong>${product.name}</strong><br><small>${formatMoney(product.price)} each</small></td>
        <td>${targetQty}</td>
        <td>${sold}</td>
        <td><strong>${qtyRemaining}</strong></td>
        <td>${formatMoney(qtyRemaining * Number(product.price || 0))}</td>
        <td><span class="target-inline-progress"><i style="width:${productPercent}%"></i></span> ${Math.round(productPercent)}%</td>
        <td><button type="button" class="cart-remove-btn" onclick="removeProductTarget('${target.id}')">Remove</button></td>
      </tr>
    `;
  }).join('') || `<tr><td colspan="7" style="text-align:center; color:var(--text-muted);">No product targets set for ${periodText.toLowerCase()}.</td></tr>`;
}

function saveRevenueTarget(event) {
  event.preventDefault();
  let period = document.getElementById('targetFormPeriod').value;
  let amount = Math.max(0, Number(document.getElementById('targetRevenueInput').value || 0));
  let targets = getSalesTargets();
  targets.revenue = { today: 0, weekly: 0, monthly: 0, ...(targets.revenue || {}) };
  targets.revenue[period] = amount;
  writeJson("salesTargets", targets);
  activeTargetPeriod = period;
  recordActivity("Revenue target updated", `${period}: ${formatMoney(amount)}`);
  renderTargetsView();
}

function saveProductTarget(event) {
  event.preventDefault();
  let sku = document.getElementById('targetProductSelect').value;
  let quantity = Math.max(1, Number(document.getElementById('targetProductQuantity').value || 1));
  let period = document.getElementById('targetProductPeriod').value;
  let targets = getSalesTargets();
  targets.products = Array.isArray(targets.products) ? targets.products : [];
  let existing = targets.products.find(target => target.sku === sku && target.period === period);
  if (existing) {
    existing.quantity = quantity;
  } else {
    targets.products.push({ id: `TARGET-${Date.now()}`, sku, period, quantity });
  }
  writeJson("salesTargets", targets);
  activeTargetPeriod = period;
  document.getElementById('targetProductQuantity').value = "";
  renderTargetsView();
}

function removeProductTarget(id) {
  let targets = getSalesTargets();
  targets.products = (targets.products || []).filter(target => target.id !== id);
  writeJson("salesTargets", targets);
  renderTargetsView();
}
...
function resolveCartProduct(item, catalog = getProductCatalog()) {
  let products = Object.values(catalog);
  return products.find(product => item.sku && product.sku === item.sku)
    || products.find(product => product.name === (item.productName || item.name))
    || null;
}

function getStockVariantKey(item) {
  return item.unit === "pcs" || item.gram === "52gm" ? "52gm__pcs" : "original";
}

function getAvailableStock(stock, productName, variantKey) {
  let value = stock[productName];
  if (value && typeof value === "object") {
    return Number(value[variantKey] || 0);
  }
  return variantKey === "original" ? Number(value || 0) : 0;
}
...
function fetchLastOrderToCart() {
  ...
  order.forEach(item => {
    ...
    appCart.push({
      ...
      sku: getProductCatalog()[name]?.sku || "",
      productName: name,
      name,
      gram,
      unit,
...
function executeSaveOrder() {
  ...
  // Resolve and validate every cart line against the latest warehouse state.
  let stock = getStoredStock();
  let catalog = getProductCatalog();
  let resolvedCart = [];
  let stockErrors = [];
  ...
  appCart.forEach(item => {
    let product = resolveCartProduct(item, catalog);
    if (!product) {
      stockErrors.push(`${item.name}: product no longer exists`);
      return;
    }
    let variantKey = getStockVariantKey(item);
    let available = getAvailableStock(stock, product.name, variantKey);
    if (available < Number(item.qty || 0)) {
      stockErrors.push(`${product.name}: requested ${item.qty}, available ${available}`);
      return;
    }
    resolvedCart.push({ item, product, variantKey, available });
  });

  if (stockErrors.length) {
    alert(`Order could not be saved because stock changed:\n${stockErrors.join("\n")}`);
    renderOrdersCatalog();
    return;
  }

  resolvedCart.forEach(({ item, product, variantKey, available }) => {
    if (typeof stock[product.name] !== "object" || stock[product.name] === null) {
      stock[product.name] = {
        original: Number(stock[product.name] || 0),
        "52gm__pcs": 0
      };
    }
    stock[product.name][variantKey] = available - Number(item.qty || 0);
...
function executeBulkAction() {
  ...
  removeJson(CRAXX_ANALYTICS_KEY);
  selectedBulkProducts.clear();
...
function loadSettingsFields() {
  ...
  let auth = getAuthSettings();
  document.getElementById('settingsSupabaseUrl').value = auth.url || "";
  document.getElementById('settingsSupabaseAnonKey').value = auth.anonKey || "";
}

function saveAuthSettings() {
  let url = document.getElementById('settingsSupabaseUrl').value.trim().replace(/\/$/, "");
  let anonKey = document.getElementById('settingsSupabaseAnonKey').value.trim();
  if (!url || !anonKey) {
    alert("Enter both the Supabase project URL and anonymous key.");
    return;
  }
  ...
  writeJson("craxxAuthConfig", { url, anonKey });
  craxxSupabaseClient = null;
  recordActivity("Email OTP configured", url);
  alert("Email OTP configuration saved. Sign out to test email verification.");
}
...
function executeDatabaseBackup() {
  let keys = [
    ...
    "systemNotifications", "craxxIdentity", "adminProfile", "salesTargets"
  ];
  
  let dbBackup = {};
  keys.forEach(k => {
    dbBackup[k] = localStorage.getItem(scopedStorageKey(k));
  });
  dbBackup.__account = {
    id: localStorage.getItem("craxxActiveUserId") || "local-admin",
    email: localStorage.getItem("craxxActiveUserEmail") || "Local Administrator"
  };
  ...
  reader.onload = event => {
    try {
      ...
      Object.keys(dbBackup).forEach(k => {
        if (k !== "__account" && dbBackup[k] !== null) {
          localStorage.setItem(scopedStorageKey(k), dbBackup[k]);
        }
      });
...
function executeDatabaseWipe() {
  if (confirm("Clear the current user's CRAXX workspace? This deletes this account's orders, products, stock, customers, and targets.")) {
    CRAXX_USER_SCOPED_KEYS.forEach(key => localStorage.removeItem(scopedStorageKey(key)));
    alert("Current user workspace cleared successfully.");
    window.location.reload();
  }
}
...
function loadProfileFields() {
  ...
  let activeEmail = localStorage.getItem("craxxActiveUserEmail");
  if (activeEmail && activeEmail !== "Local Administrator") {
    document.getElementById('adminEmailInput').value = activeEmail;
    document.getElementById('adminEmailInput').readOnly = true;
  }
}
...
function toggleMobileSidebar() {
  ...
  if (sidebar && backdrop) {
    ...
    let isOpen = sidebar.classList.contains('mobile-open');
    document.body.classList.toggle('sidebar-open', isOpen);
    document.querySelector('.mobile-menu-btn')?.setAttribute('aria-expanded', String(isOpen));
  }
}

function closeMobileSidebarOnResize() {
  if (window.innerWidth > 768) {
    document.getElementById('enterpriseSidebar')?.classList.remove('mobile-open');
    document.getElementById('sidebarBackdrop')?.classList.remove('active');
    document.body.classList.remove('sidebar-open');
  }
}

function scheduleAnalyticsCalendarRefresh() {
  let now = new Date();
  let nextDay = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, 0, 0, 1);
  window.setTimeout(() => {
    removeJson(CRAXX_ANALYTICS_KEY);
    handleHashRoute();
    scheduleAnalyticsCalendarRefresh();
...
window.addEventListener('storage', event => {
  let changedKey = event.key?.split(':').pop();
  if (['stock', 'orderHistory', 'productCatalog', 'activityFeed'].includes(changedKey)) {
    removeJson(CRAXX_ANALYTICS_KEY);
    let activeView = window.location.hash.slice(1).split('?')[0] || 'dashboard';
    initViewData(activeView, {});
  }
});

window.addEventListener('craxx:notifications', updateNotificationBadge);
